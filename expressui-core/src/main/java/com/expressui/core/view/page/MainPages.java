...
/**
 * Main pages into the ExpressUI application, presented as a Vaadin Tabsheet.
 * <p/>
 * Each page is presented as a Vaadin Tab.
 */
public abstract class MainPages {

    /**
     * Get all the pages of the application, including those the user doesn't
     * have permission to view.
     * <p/>
     * Implementer should return all pages and let ExpressUI take care of
     * security handling.
     *
     * @return all pages into the application
     */
    public abstract List<Page> getPages();

    /**
     * Get all pages that the user has security permission to view.
     *
     * @return all pages user is permitted to view
     */
    public final List<Page> getViewablePages() {
        List<Page> pages = getPages();
        List<Page> viewablePages = new ArrayList<Page>();

        for (Page page : pages) {
            if (page.isViewAllowed()) {
    ...
    public void postWire() {
        List<Page> pages = getViewablePages();
        for (Page page : pages) {
            page.postWire();
        }
    }
